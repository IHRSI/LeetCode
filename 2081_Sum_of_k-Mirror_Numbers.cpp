//Editorial - cant code this in exams as precompution is not possible manually. this approach is also limited to this Q only
// because of constrains Constraints: 2 <= k <= 9 1 <= n <= 30
//Learnt about constexpr and static and accumulate func in 2d array
class Solution {
private:
    /*The absolute space used is O(n*k)=O(240).
    But since Big-O notation ignores constants, we still describe this as O(1) space complexity.*/
    static constexpr long long ans[][30] = { //TC=O(n) , SC=O(1) as used constexpr it takers space in compile data not runtime
        {1,       3,       5,        7,         9,         33,
